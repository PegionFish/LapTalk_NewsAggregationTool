        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={tableStyle}>
            ...
            <tbody>
              {sources.map(s => (
                <>
                  <tr key={s.name} style={{ borderBottom: '1px solid var(--border)', background: s.health === 'failing' ? 'rgba(239,83,80,0.04)' : 'transparent' }}>
                    <td style={tdStyle}>
                      <button onClick={() => handleExpand(s.name)} style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: 12, padding: 0, fontWeight: 600 }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 140, overflowY: 'auto' }}>
                              {sourceHistory.map((h, i) => (
                                <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'center', fontSize: 11, fontFamily: 'monospace' }}>
                                  <span style={{ color: 'var(--text-muted)', minWidth: 130 }}>{h.started_at ? h.started_at.replace('T', ' ').substring(0, 19) : ''}</span>
