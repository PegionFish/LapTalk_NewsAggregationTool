...
def get_client() -> OpenAI:
    ...
    client = OpenAI(
        ...
        timeout=600.0,  # 600 秒 — 大上下文全景图推理可能需要 3-5 分钟
    )
...
def build_chains_panoramic(context: str) -> list[dict] | None:
    """基于全景图识别逻辑链分组。
    返回 [{events:[{id:int, role:str}, ...], title:str, reason:str}]，
    每个事件附带其在链中的角色说明，粒度控制在具体产品/架构/平台级别。
    """
    result = _ai_json(
        f"以下是当前所有活跃科技事件的全景图（含每个事件的标题、关联文章标题、发布时间和关键词）：\n\n{context}\n\n"
        "请识别应该构筑为逻辑链的事件分组。\n\n"
        "粒度要求（非常重要）：\n"
        "- 聚焦具体产品/架构/平台/技术，不是公司级别\n"
        "- 例如：「AMD Strix Halo 从 leak 到发布」而非「AMD 相关事件」\n"
        "- 例如：「NVIDIA Rubin 架构演进」而非「NVIDIA GPU 新闻」\n"
        "- 每条链 2-5 个事件为宜，最多不超过 8 个\n\n"
        "适合构筑逻辑链的模式：\n"
        "- 产品开发时间线：早期传闻/leak → 工程样品/海关记录 → 官方预告 → 正式发布 → 评测/拆解\n"
        "- 技术演进：架构公布 → 内核补丁 → 驱动支持 → 量产上市\n"
        "- 因果链：事件A导致事件B导致事件C（如安全漏洞→召回→修复→重新上市）\n"
        "- 竞品对比：同一市场定位的多个产品发布\n\n"
        "输出 JSON 数组格式：\n"
        '[{"events":[{"id":事件ID,"role":"该事件在链中的具体角色（10字以内）"},...],'
        '"title":"20字以内链标题","reason":"15字以内分组理由"}]\n\n'
        "规则：\n"
        "1. 每组至少 2 个事件\n"
        "2. 同一事件可以出现在多个链中\n"
        "3. role 字段必须具体说明该事件在链中的作用（如「海关出货」「内核补丁」「正式发布」）\n"
        "4. 仅标题相似但无实质产品/技术关联的事件不要强行分组\n"
        "5. 优先识别有明确时间先后或因果关系的序列",
        "你是资深硬件/科技新闻编辑，擅长从事件中识别具体产品和技术的发展脉络。只输出 JSON 数组，不输出其他内容。",
        max_tokens=262144,
        temperature=0.1,
        response_format=None,  # 需要数组输出，禁用 json_object 格式
    )
